...
/// Load a language's prosody data (`prosody.yaml`), if it ships one.
///
/// Only dialects that declare a `meter:` ever ask for this, so a language
/// without the file behaves exactly as it did.
fn load_prosody_uncached(language: &str) -> Option<crate::generator::prosody::ProsodyModel> {
    let content = get_embedded_yaml(&format!("{}/prosody.yaml", language))?;
    match crate::generator::prosody::ProsodyModel::from_yaml_str(content) {
        Ok(model) if !model.is_empty() => Some(model),
        Ok(_) => None,
        Err(e) => {
            #[cfg(not(target_arch = "wasm32"))]
            eprintln!("warning: ignoring {}/prosody.yaml: {}", language, e);
            let _ = e;
            None
        }
    }
...
static PROSODY_CACHE: OnceLock<Mutex<HashMap<String, Option<Arc<crate::generator::prosody::ProsodyModel>>>>> =
    OnceLock::new();

/// `load_prosody_uncached`, memoized and pre-wrapped in the `Arc` the Lexicon
/// wants. Same reasoning as the semantics cache: English's prosody.yaml is
/// ~90 KB and its parse depends only on the language.
///
/// The value is built outside the lock, so a parse panic cannot poison it.
pub fn load_prosody_cached(
    language: &str,
) -> Option<Arc<crate::generator::prosody::ProsodyModel>> {
    let cache = PROSODY_CACHE.get_or_init(|| Mutex::new(HashMap::new()));
    if let Some(hit) = cache.lock().unwrap().get(language) {
        return hit.clone();
    }
    let model = load_prosody_uncached(language).map(Arc::new);
    cache.lock().unwrap().insert(language.to_string(), model.clone());
    model
}
